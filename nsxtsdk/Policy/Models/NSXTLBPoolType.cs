...
namespace nsxtsdk.PolicyModels
{
    ...
    public class NSXTLBPoolType : NSXTPolicyConfigResourceType
    {
        ...
        [NSXTProperty(IsRequired: false, Description: @"Depending on the topology, Source NAT (SNAT) may be required to ensuretraffic from the server destined to the client is received by the loadbalancer. SNAT can be enabled per pool. If SNAT is not enabled for apool, then load balancer uses the client IP and port (spoofing) whileestablishing connections to the servers. This is referred to as no-SNATor TRANSPARENT mode.  By default Source NAT is enabled as LBSnatAutoMap.")]
